...
-- Productivity Hub: Timers module (visual countdown timers + task checklist).
create table if not exists public.timers_state (
  user_id uuid primary key references auth.users(id) on delete cascade,
  state jsonb not null,
  updated_at timestamptz not null default now()
);
...
alter table public.timers_state enable row level security;
...
drop policy if exists "timers_state owner" on public.timers_state;
create policy "timers_state owner" on public.timers_state
  for all
  using (auth.uid() = user_id)
  with check (auth.uid() = user_id);
